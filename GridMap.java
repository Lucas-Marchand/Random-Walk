...
public class GridMap extends JPanel
{
	...
	public void paintComponent(Graphics obj)
	{
		...
		// draw the current position as a rectangle
		Point where = walk.getCurrentPoint();
		...
		page.fillRect(start.x * blockSize + offset - blockSize / 8,
				start.y * blockSize + offset - blockSize / 8,
				blockSize/4, blockSize/4);

		// draw destination point
		Point end = walk.getEndPoint();
		page.setColor(Color.ORANGE);
		page.fillRect(end.x * blockSize + offset - blockSize / 8,
				end.y * blockSize + offset - blockSize / 8,
				blockSize/4, blockSize/4);
	}
